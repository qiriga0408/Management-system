  ...
  <div class="dhjl">
    ...
    <div class="w_c">
      <el-table :data="tableData" style="width:100%">
        ...
        <el-table-column prop="total_count" label="累计交易"  min-width="100"> </el-table-column>
        <el-table-column prop="total_amount" label="累计分润"  min-width="100"> </el-table-column>
        <el-table-column prop="created_time" label="入驻时间"  min-width="130">
           <template slot-scope="scope">
            <span>{{scope.row.created_time.substring(0,scope.row.created_time.indexOf(" "))}}</span><br>
            <span>{{scope.row.created_time.substring(scope.row.created_time.indexOf(" "))}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column v-if="$store.state.qx.tradersListSet" prop="" fixed="right" label="操作" width="260" > 
           <template slot-scope="scope">
            ...
            <el-button  type="primary"
              size="mini" 
              v-if="scope.row.is_status !== -1"
              @click="handleRevoke(scope.row)">撤销资格</el-button>
            <el-button  type="primary"
              size="mini" 
              v-if="[0,1].indexOf(scope.row.is_status)>-1"
              @click="handleStop(scope.row)">{{scope.row.is_status == 0 ? '开启资格' : '停用资格'}}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
     ...
     <el-dialog
      ...
      <div class="dialogc">
        ...
        <el-row :gutter="2">
          ...
          <el-col :span="20"><div class="dialogc_val">{{dialogData.created_time}}</div></el-col>
        </el-row>
        <el-row :gutter="2">
    ...
    handleStop(v){
      this.$api.setTradersState({
        tag:{
            dataType: SET_TRADERSSTATE,
            user_id: Number(v.user_id),
            stype: v.is_status == 0 ? 1 : 0,// -1注销，0停用 1正常
          },
       });
    },
