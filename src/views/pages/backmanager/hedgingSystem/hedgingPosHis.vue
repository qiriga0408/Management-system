  ...
  <div class="lsjl">
    ...
    <div id="wcdiv_lsjl" class="w_c">
      <el-table :data="tableData" style="width:100%" :cell-style="{ height: rowh + 'px' }">
        ...
        <el-table-column prop="longvalue" label="持多价值" min-width="100"></el-table-column>
        <el-table-column prop="shortvalue" label="持空价值" min-width="110"></el-table-column>
        <el-table-column prop="longprice" label="持多单开仓均价" min-width="110"></el-table-column>
        <el-table-column prop="shortprice" label="持空单开仓均价" min-width="110"></el-table-column>
...
<script>
import { BCPROHEDGEPOSITION ,HYPZ_SXF,BCPROHEDGEACCOUNTLIST} from "@/constant";
export default {
  data() {
    return {
      //合约交易对
  ...
  mounted() {
    this.$register(this);
    this.msg = [BCPROHEDGEPOSITION,HYPZ_SXF,BCPROHEDGEACCOUNTLIST];
  ...
  methods: {
    ...
    getData() {
      ...
      this.tableData = [];
      this.$api.getbcprohedgepositionhistory({
        tag: {
        ...
        case HYPZ_SXF:
          if (data.ret === 0) {
            if (data.data) {
              ...
              this.accountData = this.accountData.concat(data.data.map(v=>{
                return {
                  label:v.traderpairs,
                  value:v.traderpairs
                }
