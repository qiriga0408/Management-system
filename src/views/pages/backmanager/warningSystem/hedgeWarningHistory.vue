      ...
      <el-table :data="tableData" style="width:100%" :cell-style="{ height: rowh + 'px' }">
        <el-table-column prop="createdby" label="预警时间" min-width="180">
          <template slot-scope="scope">
            <span>{{scope.row.createdby.substring(0,scope.row.createdby.indexOf(" "))}}</span>
            <br />
            <span>{{scope.row.createdby.substring(scope.row.createdby.indexOf(" "))}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="contractcode" label="合约交易账户" min-width="180">
          <template slot-scope="scope">
            <span>{{scope.row.contractcode?scope.row.contractcode:'--'}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="hedgeaccount" label="对冲子账号" min-width="180">
          <template slot-scope="scope">
            <span>{{scope.row.hedgeaccount?scope.row.hedgeaccount:'--'}}</span>
          </template>
        </el-table-column>
...
<script>
import { BCPROHEDGENOTICE, ASSETS_COINLIST } from "@/constant";
export default {
  data() {
    return {
      typeData: [
        { label: "全部预警类型", value: 0 },
        { label: "资产类", value: 1 },
        { label: "交易类", value: 2 },
                ...
                data.data.list.map(v => {
                  ...
                  switch(v.noticetype){
                    ...
                    case 2:
                      ...
                      break;
                    case 3:
                      v.noticetype_s =  "API状态类";
                      break;
                    case 4:
                      v.noticetype_s =  "行情接口类";
                      break;
                  }
